"""Volume specification dataclass for OCIBlocks compute block volumes.

Provides :class:`VolumeSpec`, a typed descriptor for a single OCI block volume
to be created and attached to a
:class:`~blocks.compute.instance.ComputeInstance`.  Pass a list of specs to
...
class VolumeSpec:
    ...
    Attributes:
        ...
        label: Short slug used to derive the Pulumi resource name suffix
            (e.g. ``"data"``, ``"logs"``, ``"db"``).  Must start with a
            lowercase letter and contain only lowercase letters, digits, or
            hyphens.  Must be unique within the list passed to
            ``ComputeInstance``.
        vpus_per_gb: OCI volume performance-unit tier.  Use the class
            constants :attr:`PERF_LOW` (0), :attr:`PERF_BALANCED` (10,
            default), :attr:`PERF_HIGH` (20), or :attr:`PERF_ULTRA` (120).
