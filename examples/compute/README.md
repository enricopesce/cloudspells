...
Deploys a single compute instance inside a VCN. The instance is placed in the private subnet and SSH access is allowed from the public subnet.
...
- Security list rules allowing SSH on port 22
...
```bash
pulumi stack output ssh_private_key --show-secrets > ~/.ssh/oci_instance
