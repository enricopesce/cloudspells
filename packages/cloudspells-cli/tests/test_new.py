...
def test_new_vcn_creates_files(runner, tmp_path) -> None:
...
def test_new_list_flag_exits_zero_and_shows_spells(runner, tmp_path) -> None:
    with runner.isolated_filesystem(temp_dir=tmp_path):
...
def test_new_unknown_spell_exits_nonzero(runner, tmp_path) -> None:
    with runner.isolated_filesystem(temp_dir=tmp_path):
...
def test_new_missing_args_exits_nonzero(runner, tmp_path) -> None:
    with runner.isolated_filesystem(temp_dir=tmp_path):
        result = runner.invoke(app, ["new"])
        assert result.exit_code != 0


def test_new_existing_dir_blocked_without_force(runner, tmp_path) -> None:
    with runner.isolated_filesystem(temp_dir=tmp_path):
        Path("test-vcn").mkdir()
        result = runner.invoke(app, ["new", "vcn", "test-vcn"])
        assert result.exit_code != 0
...
def test_new_existing_dir_overwritten_with_force(runner, tmp_path) -> None:
    with runner.isolated_filesystem(temp_dir=tmp_path):
        Path("test-vcn").mkdir()
...
def test_new_prints_next_steps(runner, tmp_path) -> None:
    with runner.isolated_filesystem(temp_dir=tmp_path):
...
def test_new_all_spell_types(runner, tmp_path) -> None:
